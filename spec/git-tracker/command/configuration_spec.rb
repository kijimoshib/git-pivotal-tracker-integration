...
describe GitTracker::Command::Configuration do
  ...
  it 'should not prompt the user for the API token if it is already configured' do
    GitTracker::Util::Git.should_receive(:get_config).with('pivotal.api-token', :inherited).and_return('test_api_token')
  ...
  it 'should prompt the user for the API token if it is not configured' do
    GitTracker::Util::Git.should_receive(:get_config).with('pivotal.api-token', :inherited).and_return('')
    @configuration.should_receive(:ask).and_return('test_api_token')
    GitTracker::Util::Git.should_receive(:set_config).with('pivotal.api-token', 'test_api_token', :global)
  ...
  it 'should not prompt the user for the project id if it is already configured' do
    GitTracker::Util::Git.should_receive(:get_config).with('pivotal.project-id', :inherited).and_return('test_project_id')
  ...
  it 'should prompt the user for the API token if it is not configured' do
    GitTracker::Util::Git.should_receive(:get_config).with('pivotal.project-id', :inherited).and_return('')
    menu = double('menu')
    ...
    GitTracker::Util::Git.should_receive(:set_config).with('pivotal.project-id', 'test_project_id', :local)
  ...
  it 'should persist the story when requested' do
    GitTracker::Util::Git.should_receive(:set_config).with('pivotal-story-id', 12345678, :branch)
  ...
  it 'should return a story when requested' do
    ...
    story = double('story')
    GitTracker::Util::Git.should_receive(:get_config).with('pivotal-story-id', :branch).and_return('12345678')
