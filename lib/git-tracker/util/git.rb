...
class GitTracker::Util::Git
  ...
  def self.branch_name
    GitTracker::Util::Shell.exec('git branch').scan(/\* (.*)/)[0][0]
  end
  ...
  def self.create_branch(name, print_messages = true)
    ...
    GitTracker::Util::Shell.exec 'git pull --quiet --ff-only'
    puts 'OK' if print_messages

    if print_messages; print "Creating and checking out #{name}... " end
    GitTracker::Util::Shell.exec "git checkout --quiet -b #{name}"
  ...
  def self.create_commit(message, story)
    GitTracker::Util::Shell.exec "git commit --quiet --all --allow-empty --message \"#{message}\n\n[##{story.id}]\""
  end
  ...
  def self.get_config(key, scope = :inherited)
    if :branch == scope
      GitTracker::Util::Shell.exec("git config branch.#{branch_name}.#{key}", false).strip
    elsif :inherited == scope
      GitTracker::Util::Shell.exec("git config #{key}", false).strip
    else
  ...
  def self.push(*refs)
    ...
    GitTracker::Util::Shell.exec "git push --quiet #{remote} " + refs.join(' ')
    puts 'OK'
  end
  ...
  def self.set_config(key, value, scope = :local)
    if :branch == scope
      GitTracker::Util::Shell.exec "git config --local branch.#{branch_name}.#{key} #{value}"
    elsif :global == scope
      GitTracker::Util::Shell.exec "git config --global #{key} #{value}"
    elsif :local == scope
      GitTracker::Util::Shell.exec "git config --local #{key} #{value}"
    else
